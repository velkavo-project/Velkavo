...
# a main offline velkarod, does most of the tests
# a restricted RPC velkarod setup with RPC payment
# two local online velkarods connected to each other
N_VELKARODS = 5

# 4 wallets connected to the main offline velkarod
# 1 wallet connected to the first local online velkarod
# 1 offline wallet
N_WALLETS = 7
...
velkarod_base = [builddir + "/bin/velkarod", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "velkarod_p2p_port", "--rpc-bind-port", "velkarod_rpc_port", "--zmq-rpc-bind-port", "velkarod_zmq_port", "--zmq-pub", "velkarod_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "velkarod_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
velkarod_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/velkavo-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_VELKARODS):
  command_lines.append([str(18180+i) if x == "velkarod_rpc_port" else str(18280+i) if x == "velkarod_p2p_port" else str(18380+i) if x == "velkarod_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "velkarod_zmq_pub" else builddir + "/functional-tests-directory/velkarod" + str(i) if x == "velkarod_data_dir" else x for x in velkarod_base])
  if i < len(velkarod_extra):
    command_lines[-1] += velkarod_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/velkarod' + str(i) + '.log', 'a+'))
