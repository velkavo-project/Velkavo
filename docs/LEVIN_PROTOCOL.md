...
used by Velkavo. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in Velkavo's
...
> This document does not currently list all data being sent by the velkavo
> protocol, that portion is a work-in-progress. Please take the time to do it
> if interested in learning about Velkavo p2p traffic!


## Header
This header is sent for every Velkavo p2p message.
...
Velkavo when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the Velkavo specific
...
### Requests
Requests are the basis of the admin protocol for Velkavo. The `Q` bit must be
