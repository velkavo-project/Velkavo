# Anonymity Networks with Velkavo

Currently only Tor and I2P have been integrated into Velkavo. The usage of
...
When the `--proxy` flag is used, `velkarod` sends all external traffic via the
...
is made or until `velkarod` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `velkavo-wallet-cli` and
`velkavo-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
Velkarod does not support synchronizing the blockchain over onion or I2P hidden services.
You may sync the blockchain using a SOCKS4 proxy. Velkarod will connect to IPv4
nodes using this proxy to sync the blockchain.

```bash
velkarod --proxy 127.0.0.1:9050 --p2p-bind-ip 127.0.0.1 --no-igd
```
...
`--tx-proxy` which tells `velkarod` the ip/port of a socks proxy provided by a
...
which tells `velkarod` to connect to ".onion" P2P addresses using a socks
...
`--anonymous-inbound`. This option tells `velkarod` the inbound address, network
...
which tells `velkarod` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:18084" and forwarded to `velkarod`
...
forwarded to `velkarod` localhost port 18085. Using `tx-proxy`(required), these
...
`velkarod` RPC port - which is independent from the configuration for incoming
...
```text
HiddenServiceDir /var/lib/tor/data/velkavo-rpc
...
```text
# P2P Hidden service
HiddenServiceDir /var/lib/tor/data/velkavo
...
This will store key information in `/var/lib/tor/data/velkavo` and `/var/lib/tor/data/velkavo-rpc`
and will forward "Tor port" 18084 and 18089 to ports 18084 and 18089 of ip 127.0.0.1, respectively. The file
`/usr/lib/tor/data/velkavo/hostname` will contain the ".onion" address for use with `--anonymous-inbound`, and
`/var/lib/tor/data/velkavo-rpc/hostname` will contain the ".onion" address for use with RPC.

I2P must be configured with a standard server tunnel. Configuration differs by
I2P implementation.  
You can find guides for i2pd [here](https://docs.getvelkavo.org/running-node/velkarod-tori2p/#__tabbed_1_2).

## Example Node Configurations

Please check the [`velkarod` reference](https://docs.getvelkavo.org/interacting/velkarod-reference/#tori2p-and-proxies) for more information about these flags.
...
Velkarod will connect to IPv4 nodes via clearnet, revealing to your ISP
that you are running a Velkavo node, but your transactions will be relayed over
Tor.

```bash
sudo apt install tor # Or install Tor some other way
systemctl start tor # Or start Tor manually
velkarod --tx-proxy tor,127.0.0.1:9050,10
```

### Connect To IPv4 Nodes Over Tor Only

This configuration does not connect to hidden services or accept incoming
connections. Your ISP will see that you are running Tor, but not Velkarod.
...
You will need to configure [hidden services manually for Tor and I2P](https://docs.getvelkavo.org/running-node/velkarod-tori2p/#node-configuration).
...
This configuration is not currently supported. Velkarod relies on IPv4 to sync
...
system clock is fairly accurate - many peers on the Velkavo network should have
similar timestamps.
...
### Intermittent Velkavo Syncing

If a user only runs `velkarod` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `velkarod` as often as possible to conceal when transactions are being sent.
...
`velkarod` currently selects two outgoing connections every 5 minutes for
...
`velkarod` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `velkarod` should likely disconnect
