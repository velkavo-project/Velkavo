# The Current/Future Status of ZMQ in Velkavo

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Velkavo
...
matching supported by both Velkavo and ZMQ. The `format`, `context` and `event`
...
The Velkavo daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Velkavo daemon also ensures that
...
The Velkavo daemon will send a `txpool_add` pub exactly once for each
