...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/velkarod"
  then
    velkarod="$dir/velkarod"
    break
  fi
done
if test -z "$velkarod"
then
  echo "velkarod not found"
  exit 1
fi
echo "Found: $velkarod"

TORDIR="$DIR/velkavo-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting velkarod"
  tail -n 400 "$HOME/.bitvelkavo/bitvelkavo.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Velkavo\ \'" | tail -n 20
  exit 1
fi
